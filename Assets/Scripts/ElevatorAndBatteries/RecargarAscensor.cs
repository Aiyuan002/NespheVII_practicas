...
public class RecargarAscensor : MonoBehaviour
{
    ...
    public TextMeshPro textInyectar;
 
    private bool playerInRange = false;
 
    private PlayerBarkController barkController;
 
    private void Start()
    {
        barkController = FindFirstObjectByType<PlayerBarkController>();
    }
 
    private void OnEnable()
    {
        PlayerController.OnPlayerInteract += PlayerInteracted;
    }
    ...
    private void PlayerInteracted()
    {
        ...
        if (colleted == 0 || injected >= colleted)
        {
            barkController?.TriggerTransporterNoEnergyBark();
            return;
        }
